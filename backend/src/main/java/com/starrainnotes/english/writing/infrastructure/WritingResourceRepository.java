...
public class WritingResourceRepository {
    ...
    @Transactional public WritingResourceView publish(Long id){require(id);jdbc.update("UPDATE english_writing_resource SET publish_status='PUBLISHED',published_at=COALESCE(published_at,UTC_TIMESTAMP(6)) WHERE id=?",id);return get(id);}
    ...
    public boolean hasEnabledTag(Long id,String d){Integer c=jdbc.queryForObject("SELECT COUNT(*) FROM english_writing_resource_tag x JOIN english_taxonomy_term t ON t.id=x.term_id WHERE x.resource_id=? AND t.dimension=? AND t.enabled=1",Integer.class,id,d);return c!=null&&c>0;}
    public int promptReferences(Long id){Integer refs=jdbc.queryForObject("SELECT COUNT(*) FROM english_writing_prompt WHERE template_resource_id=? OR model_resource_id=?",Integer.class,id,id);return refs==null?0:refs;}
    private void replaceTags(Long id,List<Long> ids){jdbc.update("DELETE FROM english_writing_resource_tag WHERE resource_id=?",id);for(Long term:new LinkedHashSet<>(ids==null?List.of():ids)){String d=jdbc.query("SELECT dimension FROM english_taxonomy_term WHERE id=? AND enabled=1",rs->rs.next()?rs.getString(1):null,term);policy.requireTagDimension(d);jdbc.update("INSERT INTO english_writing_resource_tag(resource_id,term_id) VALUES (?,?)",id,term);}}
