...
public class WritingPublicController {
 private final WritingResourceQueryService resources;private final WritingPromptQueryService prompts;private final WritingExerciseApplicationService exercises;
 public WritingPublicController(WritingResourceQueryService resources,WritingPromptQueryService prompts,WritingExerciseApplicationService exercises){this.resources=resources;this.prompts=prompts;this.exercises=exercises;}
 ...
 @GetMapping("/prompts/{slug}/exercises") public java.util.List<ExercisePublicView> exercises(@PathVariable String slug){return exercises.publicList(slug);}
 @PostMapping("/prompts/{slug}/check") public CheckResultView check(@PathVariable String slug,@org.springframework.web.bind.annotation.RequestBody @jakarta.validation.Valid CheckAnswerRequest request){return exercises.check(slug,request);}
}
