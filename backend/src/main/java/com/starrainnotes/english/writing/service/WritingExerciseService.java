...
public class WritingExerciseService {
    ...
    public List<ExercisePublicView> publicList(String slug){Long id=publishedPrompt(slug);return jdbc.query(sql("WHERE pe.prompt_id=? AND e.publish_status='PUBLISHED' ORDER BY e.sort_order,e.id"),(rs,n)->publicView(rs),id);}
    @Transactional public CheckResultView check(String slug,CheckAnswerRequest request){Long promptId=publishedPrompt(slug);int score=0,total=0;List<CheckItemView>items=new ArrayList<>();Set<Long>unique=new HashSet<>();for(var sub:request.answers()){if(!unique.add(sub.exerciseId()))bad("The same exercise cannot be submitted twice.");EnglishExercise e=publishedBinding(promptId,sub.exerciseId());boolean ok=safety.isCorrect(e.getQuestionType(),json.valueToTree(e.getConfigJson()),sub.answer());int earned=ok?e.getScoreValue():0;score+=earned;total+=e.getScoreValue();items.add(new CheckItemView(e.getId(),ok,earned,e.getScoreValue(),e.getExplanationMarkdown()));}return new CheckResultView(score,total,items);}
    ...
    private ExerciseView get(Long promptId,Long exerciseId){return jdbc.queryForObject(sql("WHERE e.id=?"),(rs,n)->admin(rs,promptId),exerciseId);}
    private ExerciseView admin(java.sql.ResultSet rs,Long promptId)throws java.sql.SQLException{return new ExerciseView(rs.getLong("id"),promptId,rs.getString("question_type"),rs.getString("prompt_markdown"),json.convertValue(tree(rs.getString("config_json")),new TypeReference<>(){}),rs.getString("explanation_markdown"),rs.getInt("score_value"),rs.getInt("sort_order"),rs.getString("publish_status"),date(rs.getTimestamp("updated_at")));}
    private ExercisePublicView publicView(java.sql.ResultSet rs)throws java.sql.SQLException{long id=rs.getLong("id");String type=rs.getString("question_type");return new ExercisePublicView(id,type,rs.getString("prompt_markdown"),safety.sanitize(type,tree(rs.getString("config_json")),id),rs.getInt("score_value"),rs.getInt("sort_order"));}
