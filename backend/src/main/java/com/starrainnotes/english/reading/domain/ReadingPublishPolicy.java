...
public class ReadingPublishPolicy {
    ...
    public ReadingPublishPolicy(MediaPort media) {
        this.media = media;
    }
    public List<String> violations(ReadingArticle article, boolean hasTopic, boolean hasGenre) {
        ...
        if (!hasTopic) problems.add("至少需要一个主题(TOPIC)标签");
        if (!hasGenre) problems.add("至少需要一个文体(GENRE)标签");
