...
public class WordFamilyService {
 ...
 @Transactional public WordFamilyView create(WordFamilyRequest r){long id;try{jdbc.update("INSERT INTO vocabulary_word_family(head_word,slug,description) VALUES (?,?,?)",r.headWord().trim(),r.slug().trim(),clean(r.description()));id=jdbc.queryForObject("SELECT id FROM vocabulary_word_family WHERE slug=?",Long.class,r.slug().trim());}catch(DuplicateKeyException e){throw conflict();}replaceLinks(id,r.wordIds());return get(id);}
 @Transactional public WordFamilyView update(Long id,WordFamilyRequest r){get(id);try{jdbc.update("UPDATE vocabulary_word_family SET head_word=?,slug=?,description=? WHERE id=?",r.headWord().trim(),r.slug().trim(),clean(r.description()),id);}catch(DuplicateKeyException e){throw conflict();}replaceLinks(id,r.wordIds());return get(id);}
