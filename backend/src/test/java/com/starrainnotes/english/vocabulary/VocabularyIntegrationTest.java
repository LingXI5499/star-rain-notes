...
class VocabularyIntegrationTest extends AbstractAuthIntegrationTest {
    ...
    @Test
    void publicLayerCompatibilityRouteReturnsAllSixtyThemes() throws Exception {
        for (int i = 0; i < 60; i++) {
            insertTheme("基础通用词层", 1, "兼容主题" + i);
        }

        mockMvc.perform(get("/api/v1/public/vocabulary/layers"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].themes.length()").value(60));
    }
