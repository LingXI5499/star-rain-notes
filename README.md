...
**v1.5.0 — V1 体验升级与作品静态原型展示**

本版本完成前后台响应式体验升级，并将作品展示从截图预览升级为安全的多页面静态原型。管理员可在作品页上传或从媒体库绑定 ZIP 原型包，发布后通过沙箱在线访问。标签 `v1.5.0` 包含对应前后端源码、数据库迁移、部署配置与本文档。
...
后端产物为 `backend/target/star-rain-notes-backend-1.5.0.jar`。生产启动设置 `SPRING_PROFILES_ACTIVE=prod`，由部署环境注入数据库及邮件凭据。
