...
本版本完善作品集截图放大预览：多张截图可通过清晰的左右按钮或键盘方向键前后切换，支持首尾循环与移动端操作。标签 `v1.4.1` 包含对应前后端源码、统一版本元数据与本文档。
...
- 作品集截图放大预览增加上一张、下一张按钮；支持触控友好的移动端按钮与首尾循环。
- 放大预览支持键盘左右方向键切换，并为按钮和计数补充无障碍标签。
...
后端产物为 `backend/target/star-rain-notes-backend-1.4.1.jar`。生产启动设置 `SPRING_PROFILES_ACTIVE=prod`，由部署环境注入数据库及邮件凭据。
