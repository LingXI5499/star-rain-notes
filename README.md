...
**v1.4.0 — 公式、多语言代码组与封面 UI 精简**

本版本在 v1.3.0 视觉主题 UI v3 之上，修复行内数学公式定界、新增力扣式多语言代码组（`::: code-group`），并去掉非作品模块的封面展示与编辑选项。标签 `v1.4.0` 包含对应前后端源码、统一版本元数据与本文档。
...
后端产物为 `backend/target/star-rain-notes-backend-1.4.0.jar`。生产启动设置 `SPRING_PROFILES_ACTIVE=prod`，由部署环境注入数据库及邮件凭据。
