...
describe('SEO manager', () => {
  ...
  it('uses the configured/current host and removes tracking parameters', () => {
    expect(canonicalUrl('/blog/123?utm_source=test&ref=home&keep=yes'))
      .toBe(`${window.location.origin}/blog/123?keep=yes`)
  })
  ...
  it('keeps one copy of every managed tag', () => {
    ...
    expect(document.querySelector('meta[property="og:url"]')?.getAttribute('content')).toBe(`${window.location.origin}/blog/2`)
  })
  ...
  it('marks non-indexable routes without producing query canonicals', () => {
    ...
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toBe(`${window.location.origin}/search`)
  })
})
