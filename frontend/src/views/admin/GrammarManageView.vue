...
const drawer=ref(false);const courseForm=reactive({title:'',subtitle:'',summary:'',introduction:'',roadmapMarkdown:'',coverMediaId:null as number|null})
function openCourse(){const c=data.value?.course;if(!c)return;Object.assign(courseForm,{title:c.title,subtitle:c.subtitle??'',summary:c.summary??'',introduction:c.introduction??'',roadmapMarkdown:c.roadmapMarkdown??'',coverMediaId:c.coverMediaId});drawer.value=true}
