...
const drawer=ref(false);const picker=ref(false);const courseForm=reactive({title:'',subtitle:'',summary:'',introduction:'',roadmapMarkdown:'',coverMediaId:null as number|null,coverUrl:'',seoTitle:'',seoDescription:''})
function openCourse(){const c=data.value?.course;if(!c)return;Object.assign(courseForm,{title:c.title,subtitle:c.subtitle??'',summary:c.summary??'',introduction:c.introduction??'',roadmapMarkdown:c.roadmapMarkdown??'',coverMediaId:c.coverMediaId,coverUrl:c.coverUrl??'',seoTitle:c.seoTitle??'',seoDescription:c.seoDescription??''});drawer.value=true}
async function saveCourse(){try{await updateGrammarCourse({...courseForm,subtitle:courseForm.subtitle||null,summary:courseForm.summary||null,introduction:courseForm.introduction||null,roadmapMarkdown:courseForm.roadmapMarkdown||null,seoTitle:courseForm.seoTitle||null,seoDescription:courseForm.seoDescription||null});drawer.value=false;await load(activeId.value);ElMessage.success('课程信息已保存。')}catch(e){problem(e)}}
