...
const dialogOpen = ref(false); const saving = ref(false); const editingId = ref<number | null>(null); const mediaOpen = ref(false)
const form = reactive({ ruleType: 'LINKING', title: '', slug: '', summary: '', bodyMarkdown: '', audioMediaId: null as number|null, audioUrl: null as string|null, sortOrder: 10 })
...
function openRouteEditor(){
  ...
  if(route.name==='admin-listening-pronunciation-edit'){
    ...
    editingId.value=r.id; Object.assign(form,{ruleType:r.ruleType,title:r.title,slug:r.slug,summary:r.summary,bodyMarkdown:r.bodyMarkdown,audioMediaId:r.audioMediaId,audioUrl:r.audioUrl,sortOrder:r.sortOrder}); dialogOpen.value=true
  }
}
...
<template>
  <section class="pr-m"><header class="pr-m__hero"><div><p>PRONUNCIATION · 语音规则</p><h1>语音规则</h1><span>连读、弱读、同化、省音、重音、语调六类独立教学。</span></div><el-button type="primary" @click="openCreate">新建规则</el-button></header>
    <div v-loading="loading" class="pr-m__groups">
      <div v-for="g in grouped" :key="g.type" class="pr-group"><h2>{{ ruleLabel[g.type] }} <small>{{ g.type }}</small></h2>
        ...
        <article v-for="r in g.items" :key="r.id" class="pr-card"><div class="pr-card__head"><h3>{{ r.title }}</h3><span class="pr-card__status">{{ r.publishStatus }}</span></div><p class="pr-card__summary">{{ r.summary }}</p>
          <div class="pr-card__actions"><el-button link type="primary" @click="openEdit(r)">编辑</el-button><el-button v-if="r.publishStatus==='PUBLISHED'" link @click="router.push(`/english/listening/pronunciation/${r.slug}`)">预览</el-button><el-button v-if="r.publishStatus!=='PUBLISHED'" link type="success" @click="setPub(r,true)">发布</el-button><el-button v-else link type="warning" @click="setPub(r,false)">撤回</el-button><el-button link type="danger" @click="remove(r)">删除</el-button></div>
        </article>
      </div>
    </div>
    <el-dialog v-model="dialogOpen" :title="editingId?'编辑规则':'新建规则'" width="min(860px, calc(100vw - 32px))" :close-on-click-modal="false" @closed="route.name!=='admin-listening-pronunciation'&&closeDialog()"><el-form label-position="top">
      ...
      <el-form-item label="示例音频"><audio v-if="form.audioUrl" :src="form.audioUrl" controls preload="none" class="pr-m__audio"/><div><el-button @click="mediaOpen=true">选择音频</el-button><el-button v-if="form.audioMediaId" @click="form.audioMediaId=null;form.audioUrl=null">移除</el-button></div></el-form-item>
      <el-form-item label="排序"><el-input-number v-model="form.sortOrder" :min="1" :step="10"/></el-form-item>
    </el-form><template #footer><el-button @click="closeDialog">取消</el-button><el-button type="primary" :loading="saving" @click="save">保存</el-button></template></el-dialog>
