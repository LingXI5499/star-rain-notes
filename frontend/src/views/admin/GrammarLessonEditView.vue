...
const form=reactive({sectionId:null as number|null,title:'',summary:'',bodyMarkdown:'## 学习内容\n\n'})
...
onMounted(async()=>{try{const curriculum=await fetchGrammarCurriculum();sections.value=curriculum.sections;if(editing.value&&lessonId){const item=await fetchGrammarLesson(lessonId);Object.assign(form,{sectionId:item.sectionId,title:item.title,summary:item.summary??'',bodyMarkdown:item.bodyMarkdown})}else{const id=Number(route.query.section);form.sectionId=sections.value.some(s=>s.id===id)?id:sections.value[0]?.id??null}}catch{ElMessage.error('课节加载失败。')}finally{loading.value=false;capture()}})
async function save(){if(!form.sectionId||!form.title.trim()||!form.bodyMarkdown.trim()){ElMessage.warning('请填写章节、标题和正文。');return}saving.value=true;try{const payload={sectionId:form.sectionId,title:form.title.trim(),summary:form.summary||null,bodyMarkdown:form.bodyMarkdown};const result=editing.value&&lessonId?await updateGrammarLesson(lessonId,payload):await createGrammarLesson(payload);capture();ElMessage.success(isReview(result)?'已提交审核，超级管理员批准后会应用到线上课节。':'课节已保存。');back()}catch(error){const p=error instanceof AxiosError?error.response?.data as ProblemDetail:null;ElMessage.error(p?.detail??'保存失败。')}finally{saving.value=false}}
</script>
