...
const readingStats = computed(() => estimateReadingStats(post.value?.bodyMarkdown))
...
<template>
  <section v-if="(notFound || loadFailed) && !post" class="article-state">
    <strong>{{ notFound ? '文章不存在或尚未公开' : '加载失败，请稍后重试' }}</strong>
    <RouterLink to="/blog">返回博客</RouterLink>
  </section>
  <article v-else-if="post" class="article" :class="{ 'is-swapping': swapping }" :aria-busy="swapping">
    <header class="article-hero">
      ...
      <div class="article-hero__tags">
        <RouterLink v-for="tag in post.tags" :key="tag.id" :to="{ path: '/blog', query: { tag: tag.slug } }"># {{ tag.name }}</RouterLink>
      </div>
      <h1>{{ post.title }}</h1>
      <p>{{ post.summary }}</p>
    ...
    <div class="article__reading">
      <main class="article__body">
        ...
        <nav class="article-nav">
          <RouterLink v-if="post.previous" :to="`/blog/${post.previous.slug}`">
            <small>上一篇</small>
            <strong>← {{ post.previous.title }}</strong>
          </RouterLink>
          <span v-else />
          <RouterLink v-if="post.next" :to="`/blog/${post.next.slug}`" class="next">
            <small>下一篇</small>
...
.article.is-swapping .article__reading {
  opacity: 0.45;
  pointer-events: none;
  transition: opacity var(--motion-fast, 140ms) var(--ease-standard, ease);
}
