...
const items = ref<BundleItem[]>([])
const records = ref<Record<string, LearningRecord | null>>({})
...
const moduleCounts = computed(() =>
  items.value.reduce(
    ...
    { READING: 0, LISTENING: 0, WRITING: 0 } as Record<BundleItem['contentType'], number>,
  ),
)
function key(item: BundleItem) {
  return `${item.contentType}-${item.contentId}`
}
function target(item: BundleItem) {
  return item.contentType === 'READING'
    ? `/english/reading/${item.slug}`
    : item.contentType === 'LISTENING'
      ? `/english/listening/${item.slug}`
...
async function load() {
  ...
  try {
    ...
    const [meta, list] = await Promise.all([fetchPublicBundle(slug), fetchPublicBundleItems(slug)])
    if (!isCurrent(version)) return
    bundle.value = meta
    items.value = list
    applyPageMeta({ title: meta.title, description: meta.summary || undefined })
    const batch = await fetchLearningRecords(
      list.map((item) => ({ contentType: item.contentType, contentId: item.contentId })),
    ).catch(() => ({}) as Record<string, LearningRecord>)
    if (!isCurrent(version)) return
    records.value = Object.fromEntries(
      list.map((item) => [key(item), batch[`${item.contentType}:${item.contentId}`] ?? null]),
    )
  } catch {
    if (!isCurrent(version)) return
    failed.value = true
...
<template>
  <section class="bundle-detail">
    <p v-if="initialLoading && !bundle" class="state">正在准备学习路径…</p>
