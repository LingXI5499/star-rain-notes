      ...
      <div class="insights-grid">
        ...
        <section class="panel recommendation-panel">
          ...
          <div v-if="insights.recommendations.length" class="recommendations">
            <RouterLink v-for="item in insights.recommendations" :key="`${item.contentType}-${item.contentId}`" :to="item.route">
              <div><span>{{ item.reason }} · {{ moduleLabels[item.contentType] }}<template v-if="item.cefrLevel"> · {{ item.cefrLevel }}</template></span><strong>{{ item.title }}</strong></div>
              <em>继续 →</em>
            </RouterLink>
          </div>
...
<style scoped>
.insights-page{max-width:1180px;margin:auto}.insights-hero{display:flex;justify-content:space-between;align-items:end;gap:30px;padding:22px 0 36px;border-bottom:1px solid var(--border)}.insights-hero p,.panel header small{color:var(--accent);font-size:11px;font-weight:800;letter-spacing:.15em}.insights-hero h1{max-width:760px;margin:9px 0 12px;font-size:clamp(32px,5vw,54px);line-height:1.12}.insights-hero span{display:block;max-width:720px;color:var(--text-secondary);line-height:1.7}.back-link{flex:none;border:1px solid var(--border);border-radius:999px;padding:10px 16px;color:var(--text-secondary)}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:28px 0}.metrics article,.panel{border:1px solid var(--border);border-radius:20px;background:var(--bg-surface)}.metrics article{display:flex;flex-direction:column;padding:20px;background:linear-gradient(145deg,var(--bg-surface),color-mix(in srgb,var(--primary) 5%,var(--bg-surface)))}.metrics small{color:var(--text-muted)}.metrics strong{margin:12px 0 8px;font-size:26px}.metrics span{color:var(--text-secondary);font-size:12px}.insights-grid{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(320px,.65fr);gap:18px}.panel{padding:22px}.panel>header{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:22px}.panel h2{margin:5px 0 0;font-size:21px}.panel header>span,.panel header>a{color:var(--text-muted);font-size:12px}.activity-chart{display:grid;grid-template-columns:repeat(14,minmax(24px,1fr));gap:8px;height:210px}.activity-day{display:grid;grid-template-rows:20px 1fr 22px;gap:5px;text-align:center}.activity-value{font-size:10px;color:var(--primary)}.activity-track{display:flex;align-items:end;justify-content:center;border-radius:10px;background:var(--bg-subtle);overflow:hidden}.activity-track i{display:block;width:100%;min-height:3px;border-radius:9px;background:linear-gradient(180deg,var(--accent),var(--primary));transition:height .16s ease}.activity-day small{font-size:9px;color:var(--text-muted);writing-mode:vertical-rl}.recommendations{display:flex;flex-direction:column;gap:9px}.recommendations a{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px;border-radius:13px;background:var(--bg-subtle);color:inherit;transition:transform .16s ease,background .16s ease}.recommendations a:hover{transform:translateX(3px);background:color-mix(in srgb,var(--primary) 8%,var(--bg-subtle))}.recommendations span{display:block;color:var(--accent);font-size:10px}.recommendations strong{display:block;margin-top:4px;font-size:14px}.recommendations em{flex:none;color:var(--primary);font-size:12px;font-style:normal}.module-panel{margin-top:18px}.modules{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}.modules article{padding:15px;border-radius:14px;background:var(--bg-subtle)}.modules article>div:first-child{display:flex;justify-content:space-between;gap:10px}.modules article span,.modules article small{color:var(--text-muted);font-size:11px}.mastery{height:6px;margin:16px 0 10px;border-radius:999px;background:var(--border);overflow:hidden}.mastery i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--primary),var(--accent))}.state,.empty{padding:80px 0;text-align:center;color:var(--text-muted)}@media(max-width:900px){.metrics,.modules{grid-template-columns:repeat(2,1fr)}.insights-grid{grid-template-columns:1fr}}@media(max-width:600px){.insights-hero{align-items:flex-start;flex-direction:column}.metrics,.modules{grid-template-columns:1fr}.activity-chart{gap:4px}.panel{padding:16px}.activity-day small{font-size:8px}}@media(prefers-reduced-motion:reduce){.activity-track i,.recommendations a{transition:none}}
</style>
