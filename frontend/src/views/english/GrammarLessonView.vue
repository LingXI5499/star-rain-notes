...
const readMinutes=computed(()=>Math.max(1,Math.round((lesson.value?.bodyMarkdown.length??0)/400)))
...
<template>
  ...
  <section v-else class="grammar-reader">
    ...
    <article :class="{'is-loading':lessonLoading}" :aria-busy="lessonLoading"><nav class="breadcrumb"><RouterLink to="/english">英语</RouterLink><span>/</span><RouterLink to="/english/grammar">语法教程</RouterLink><span>/</span><span>{{ lesson.sectionTitle }}</span></nav><header><p>GRAMMAR LESSON · {{ lesson.slug }}</p><h1>{{ lesson.title }}</h1><span>{{ lesson.summary }}</span><small>预计阅读 {{ readMinutes }} 分钟 · 课程进度 {{ progress }}%</small></header><MarkdownRenderer :source="lesson.bodyMarkdown" @outline="outline=$event"/><section class="lesson-complete"><div><small>LEARNING RECORD</small><strong>{{ learningRecord?.status==='COMPLETED'?'课节已完成':'完成本课并更新进度' }}</strong><span>完成后会加入学习趋势和复习计划。</span></div><button type="button" :disabled="learningRecord?.status==='COMPLETED'" @click="completeLesson">{{ learningRecord?.status==='COMPLETED'?'已完成 ✓':'标记完成' }}</button></section><nav class="prev-next"><RouterLink v-if="lesson.previous" :to="`/english/grammar/${lesson.previous.slug}`"><small>上一篇</small><strong>← {{ lesson.previous.title }}</strong></RouterLink><span/><RouterLink v-if="lesson.next" :to="`/english/grammar/${lesson.next.slug}`" class="next"><small>下一篇</small><strong>{{ lesson.next.title }} →</strong></RouterLink></nav></article>
    <aside class="page-outline" :class="{open:outlineOpen}"><ArticleOutline :items="outline"/><div class="study-card"><p>当前章节</p><strong>{{ lesson.sectionTitle }}</strong><RouterLink to="/english/grammar">查看完整路线 →</RouterLink></div></aside>
  </section>
</template>

<style scoped>
.reader-state{padding:80px;text-align:center;color:var(--text-muted)}.grammar-reader{display:grid;grid-template-columns:minmax(230px,275px) minmax(0,780px) minmax(170px,220px);justify-content:center;gap:clamp(20px,3vw,44px);align-items:start}.course-nav{position:sticky;top:calc(var(--header-height) + 22px);max-height:calc(100vh - var(--header-height) - 44px);overflow:auto;padding:18px;border:1px solid var(--border);border-radius:17px;background:var(--bg-surface)}.course-nav__title{display:block;color:var(--text-primary);font-weight:750;margin-bottom:14px}.progress{height:5px;border-radius:99px;background:var(--bg-subtle);overflow:hidden}.progress i{display:block;height:100%;background:var(--primary);transition:width .16s ease}.course-nav>small{display:block;color:var(--text-muted);font-size:11px;margin:6px 0 18px}.course-nav section{margin-bottom:16px}.course-nav h2{font-size:12px;color:var(--text-muted);margin-bottom:5px}.course-nav section a{display:flex;gap:8px;padding:7px 8px;border-radius:8px;color:var(--text-secondary);font-size:12px;line-height:1.45}.course-nav section a span{font-family:monospace;color:var(--text-muted)}.course-nav section a:hover,.course-nav section a.active{background:color-mix(in srgb,var(--primary) 9%,transparent);color:var(--primary)}.grammar-reader>article{min-width:0}.breadcrumb{display:flex;gap:8px;font-size:13px;color:var(--text-muted);margin-bottom:24px}.breadcrumb a{color:var(--primary)}article>header{padding-bottom:25px;margin-bottom:30px;border-bottom:1px solid var(--border)}article>header p{color:var(--accent);font-size:11px;font-weight:750;letter-spacing:.13em}article>header h1{font-size:clamp(34px,4vw,48px);line-height:1.15;margin:9px 0 13px}article>header>span{display:block;color:var(--text-secondary);line-height:1.7}article>header small{display:block;margin-top:12px;color:var(--text-muted)}article :deep(.markdown-body){font-size:16px;line-height:1.9}.page-outline{position:sticky;top:calc(var(--header-height) + 22px)}.study-card{margin-top:24px;padding:15px;border:1px solid var(--border);border-radius:13px}.study-card p{color:var(--text-muted);font-size:11px}.study-card strong{display:block;font-size:13px;margin:7px 0 12px}.study-card a{font-size:12px;color:var(--primary)}.prev-next{display:grid;grid-template-columns:1fr auto 1fr;gap:15px;margin-top:48px;padding-top:22px;border-top:1px solid var(--border)}.prev-next a{display:flex;flex-direction:column;gap:5px;color:var(--text-primary)}.prev-next small{color:var(--text-muted)}.prev-next .next{text-align:right}.mobile-tools{display:none}@media(max-width:1050px){.grammar-reader{grid-template-columns:230px minmax(0,1fr)}.page-outline{display:none}}@media(max-width:720px){.grammar-reader{display:block}.mobile-tools{position:sticky;top:var(--header-height);z-index:8;display:flex;gap:8px;padding:8px 0;background:var(--bg-page)}.mobile-tools button{padding:8px 12px;border:1px solid var(--border);border-radius:9px;background:var(--bg-surface);color:var(--text-primary)}.course-nav{display:none;position:fixed;z-index:10;inset:calc(var(--header-height) + 52px) 14px 18px;max-height:none}.course-nav.open{display:block}.page-outline.open{display:block;position:fixed;z-index:10;inset:calc(var(--header-height) + 52px) 14px auto;padding:18px;border:1px solid var(--border);border-radius:15px;background:var(--bg-surface)}.study-card{display:none}}@media(prefers-reduced-motion:reduce){.progress i{transition:none}}
